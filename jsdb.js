...
*		db.set(key [string], value [any data type])
*			- db.set('foo', {a:'bar',b:{c:'baz'}});
*			- db.set('foo', 'bar');
*			- db.set('foo', ['a','b','c']);
*	
...
*		db.remove_item(key [string], index [integer|string]);
*			- db.set('foo', ['a','b','c']);
*   		  db.remove_item('foo', 1);
*   		  db.get('foo');
*   		  //returns ['a','c']
*   		
*			- db.set('foo', {a:'bar',b:{c:'baz'}});
*   		  db.remove_item('foo', 'a');
*   		  db.get('foo');
*   		  //returns {b:{c:'baz'}}
*
	...
	return {
		...
		remove_item:function( key, ind ) {
			...
			if (( typeof temp_o === "object" ) && ( typeof temp_o.splice !== "function" )) {
				var keval = eval( "temp_o." + ind + ";");
				is_object = true;
			}	
			if ( typeof temp_o.splice === "function" ) {
				temp_o.splice( ind, 1 );
			} else if ( is_object ) {
				eval( "delete temp_o." + ind + ";" );
			} else {
				this.remove( key )
			}
